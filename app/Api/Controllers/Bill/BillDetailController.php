...
class BillDetailController extends BaseController
{
		...
		$subQuery = $this->billService->billDetailModel()
			...
			->where(function ($q) use ($request) {
				...
				if ($request->start_date && $request->end_date) {
					$q->whereBetween('charge_date', [$request->start_date, $request->end_date]);
				} else if (!$request->year) {
					$q->where('charge_date', '<=', date('Y-m-t'));
				}
	...
	public function verify(Request $request)
	{
		...
		if ($request->verify_amount > $availableAmt) {
			return $this->error("核销金额大于收款可用金额！");
		}
