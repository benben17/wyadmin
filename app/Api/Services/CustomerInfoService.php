...
use App\Api\Models\Customer\CustomerBaseInfo;
use App\Api\Models\Customer\Customer as CustomerModel;
use App\Api\Models\Customer\SkyeyeLog as SkyeyeLogModel;
...
class CustomerInfoService
{
  ...
  public function save($DA,$type = 1){
    ...
    if ($type == 1) {
      $company = new CustomerBaseInfo;
      $company->skyeye_id = isset($DA['id']) ? $DA['id'] :0; //天眼查ID
    }else{
      $company = CustomerBaseInfo::find($DA['id']);
  ...
  public function getById($id){
      $companyInfo = CustomerBaseInfo::find($id);
      if ($companyInfo) {
  ...
  public function getByName($companyName){
      $companyInfo = CustomerBaseInfo::where('name',$companyName)->first();
      return $companyInfo;
  }

  public function getCompanyInfo($companyName,$user){
    $skyeyeLog = SkyeyeLogModel::where('search_name',$companyName)->first();
    if (!$skyeyeLog) {
  ...
  */
  public function skyeyeSearchCount($companyId){
    $searchCount = SkyeyeLogModel::where('company_id',$companyId)->count();
    return $searchCount;
  }
